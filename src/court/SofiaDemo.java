...
import court.jurists.InvalidJuryExperience;
import court.jurists.InvalidJudgeExperience;
import court.jurists.InvalidLawyerCases;
import court.jurists.InvalidProcecutorExperience;
...
public class SofiaDemo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < 2; i++) {
				procecutor.add(new Procecutor("Procecutor " + (i + 1), 10, 320));
			}
			...
			for (int i = 0; i < 2; i++) {
				...
				cases.add(new CivilCase(judge.get(random.nextInt(judge.size())), jury,
						accusers.get(random.nextInt(accusers.size())), witness,
			...
			for (int i = 0; i < 2; i++) {
				...
				cases.add(new CriminalCase(judge.get(random.nextInt(judge.size())), jury,
						procecutor.get(random.nextInt(procecutor.size())), witness,
						defendants.get(random.nextInt(defendants.size()))));
			}
		...
		} catch (InvalidJudgeExperience | InvalidProcecutorExperience | InvalidLawyerCases | InvalidJuryExperience e) {
